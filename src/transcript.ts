/**
 * Transcript rendering — inlined from pi-permission-auto-review's transcript.ts.
 *
 * This module reconstructs the exact JSONL transcript that the auto-review
 * classifier receives, so the probe can show or replay the real evidence.
 *
 * We inline rather than import because the package isn't guaranteed resolvable
 * from the extension loader at development time, and we only need the
...
type NormalizedValue = string | number | boolean | NormalizedValue[] | { [key: string]: NormalizedValue }

function normalizeAnswer(value: unknown): NormalizedValue {
  if (value === null || ['string', 'number', 'boolean'].includes(typeof value)) return value as NormalizedValue
...
function normalizedAnswerEvidence(answer: UserInteractionAnswer): unknown | undefined {
  ...
  if (primaryAnswer === undefined) return notes
  if (notes === undefined) return primaryAnswer
...
function normalizedUserInteraction(
  ...
  if (
    ...
    return undefined
  if (message.details === null || typeof message.details !== 'object') return undefined
  const details = message.details as UserInteractionDetails
  if (details.cancelled !== false || !Array.isArray(details.answers) || details.answers.length === 0) return undefined
  const answers: Array<{ question: string; answer: unknown }> = []
  for (const rawAnswer of details.answers) {
    if (rawAnswer === null || typeof rawAnswer !== 'object') return undefined
    const answer = rawAnswer as UserInteractionAnswer
    const answerEvidence = normalizedAnswerEvidence(answer)
    if (typeof answer.question !== 'string' || answer.question.length === 0 || answerEvidence === undefined) return undefined
    answers.push({ question: answer.question, answer: answerEvidence })
  }
...
function assistantEntries(
  ...
  if (text) entries.push({ index, kind: 'assistant', label: 'assistant', text })
  for (const rawBlock of content) {
    ...
    entries.push({ index, kind: 'tool', label: `tool:${name}`, text: serializeUnknown(block.arguments) })
  }
  return entries
}
  ...
  switch (message.role) {
    ...
    case 'toolResult': {
      ...
      if (userInteraction !== undefined) {
        return [{ index, kind: 'user_interaction', label: `user_interaction:${name}`, text: userInteraction }]
      }
    ...
    case 'bashExecution': {
      ...
      return [{ index, kind: 'tool', label: 'tool:user-bash', text: `${command}\n${output}` }]
    }
    case 'branchSummary':
...
function collectTranscriptEntries(sessionEntries: SessionEntry[]): TranscriptEntry[] {
  const interactionToolCalls = new Map<string, string>()
  return sessionEntries.flatMap((entry, index) => {
    if (entry.type === 'message') return entriesFromMessage(entry.message as MessageLike, index, interactionToolCalls)
    if (entry.type === 'compaction' || entry.type === 'branch_summary') {
      return [{ index, kind: 'assistant' as const, label: entry.type, text: entry.summary }]
    }
    if (entry.type === 'custom_message') {
      const text = textFromContent(entry.content)
      return text ? [{ index, kind: 'assistant' as const, label: 'custom', text }] : []
    }
    return []
  })
}
...
function pretruncate(entry: TranscriptEntry): TranscriptEntry {
  ...
  if (
    JSON.stringify({ index: entry.index, source: entry.kind, label: entry.label, content: entry.text }).length <=
    maxCharacters
  )
    return entry
  let lower = 0
...
export function renderTranscript(sessionEntries: SessionEntry[]): RenderedTranscript {
  ...
  const retained = [...selected].sort((l, r) => l.index - r.index)
  ...
  return {
    ...
    stats: {
      ...
      transcriptEntriesTruncated: retained.filter((e) => e.truncated === true).length,
